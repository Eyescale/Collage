...
static const std::string payload(
    "Hi! I am your payload. Lorem ipsum dolor sit amet, consectetur adipiscing "
    "elit. Ut eget felis sed leo tincidunt dictum eu eu felis. Aenean aliquam "
    "augue nec elit tristique tempus. Pellentesque dignissim adipiscing "
    "tellus, ut porttitor nisl lacinia vel. Donec malesuada lobortis velit, "
    "nec lobortis metus consequat ac. Ut dictum rutrum dui. Pellentesque quis "
    "risus at lectus bibendum laoreet. Suspendisse tristique urna quis urna "
    "faucibus et auctor risus ultricies. Morbi vitae mi vitae nisi adipiscing "
    "ultricies ac in nulla. Nam mattis venenatis nulla, non posuere felis "
    "tempus eget. Cras dapibus ultrices arcu vel dapibus. Nam hendrerit "
    "lacinia consectetur. Donec ullamcorper nibh nisl, id aliquam nisl. Nunc "
    "at tortor a lacus tincidunt gravida vitae nec risus. Suspendisse potenti. "
    "Fusce tristique dapibus ipsum, sit amet posuere turpis fermentum nec. Nam "
    "nec ante dolor.");
...
private:
    ...
    {
        TEST(_gotAsync);
        command.getNode()->send(CMD_DATA_REPLY) << command.get<uint32_t>()
                                                << ++_counter;
...
int main(int argc, char** argv)
{
    ...
    std::cout << "Async command: " << asyncTime / float(NCOMMANDS)
              ...
              << " ms, with " << payload.size()
              << "b payload: " << syncTimePayload / float(NCOMMANDS + 1)
              << std::endl;
