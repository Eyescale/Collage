...
{
public:
    explicit Reader(co::ConnectionPtr connection)
...
{
public:
    explicit Latency(co::ConnectionPtr connection)
        : connection_(connection)
    {
        TEST(start());
    }
...
bool _initialize(co::ConnectionDescriptionPtr desc, co::ConnectionPtr& listener,
...
int main(int argc, char** argv)
{
    ...
    for (size_t i = 0; types[i] != co::CONNECTIONTYPE_NONE; ++i)
    {
        ...
        std::cout << desc->type << ": "
                  << (numBW + 1) * PACKETSIZE / mFactor / bwTime << " MBps, "
                  << (sequence + 1) / mFactor / latencyTime << " Mpps"
                  << std::endl;
