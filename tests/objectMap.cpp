...
protected:
    ...
    virtual void applyInstanceData(co::DataIStream& is) { is >> message; }
...
int main(int argc, char** argv)
{
...

    {
        ...
        clientFoo = static_cast<Foo*>(client->objectMap.map(masterFoo.getID()));
        TEST(clientFoo->message == "hello foo");

        // Test map() of instance
        Bar clientBar;
        TEST(client->objectMap.map(masterBar.getID(), &clientBar) ==
             &clientBar);
