...
#define UDTLASTERROR(msg)                                           \
    (msg) << " : " << UDT::getlasterror().getErrorMessage() << " (" \
...
bool UDTConnection::connect()
{
    ...
    if (!setSockOpt(UDT_RCVSYN, static_cast<const void *>(&OFF), sizeof(OFF)))
        goto err;
...
ConnectionPtr UDTConnection::acceptSync()
{
    ...
    static const bool OFF = false;
    if (newConnection->setSockOpt(UDT_RCVSYN, static_cast<const void *>(&OFF),
                                  sizeof(OFF)))
    {
...
void UDTConnection::readNB(void *, const uint64_t)
{ /* NOP */
}

// caller: application
int64_t UDTConnection::readSync(void *buffer, const uint64_t bytes, const bool)
{
...
bool UDTConnection::tuneSocket()
{
    ...
    */
    ...
    if (!setSockOpt(UDT_SNDTIMEO, static_cast<const void *>(&timeout),
                    sizeof(timeout)))
        goto err;
    if (!setSockOpt(UDT_RCVTIMEO, static_cast<const void *>(&timeout),
                    sizeof(timeout)))
        goto err;
    /*
    ...
    */
    ...
    if (!setSockOpt(UDT_CC, static_cast<const void *>(
                                new CCCFactory<CONGESTION_CONTROL_CLASS>),
