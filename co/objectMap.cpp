...
bool ObjectMap::isDirty() const
{
    ...
    for (ObjectsCIter i = _impl->masters.begin(); i != _impl->masters.end();
         ++i)
        if ((*i)->isDirty())
            return true;
    return false;
...
bool ObjectMap::deregister(Object* object)
{
    ...
    ObjectsIter masterIt =
        std::find(_impl->masters.begin(), _impl->masters.end(), object);
...
Object* ObjectMap::map(const uint128_t& identifier, Object* instance)
{
    ...
    Object* object =
        instance ? instance : _impl->factory.createObject(entry.type);
    LBASSERT(object);
    if (!object)
        return 0;

    const uint32_t req =
        _impl->handler.mapObjectNB(object, identifier, entry.version,
                                   getMasterNode());
