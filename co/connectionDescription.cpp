...
{
    return type == rhs.type && bandwidth == rhs.bandwidth && port == rhs.port &&
           hostname == rhs.hostname && interfacename == rhs.interfacename &&
...
std::ostream& operator<<(std::ostream& os, const ConnectionDescription& desc)
{
    os << lunchbox::disableFlush << lunchbox::disableHeader << "connection"
       << std::endl
       << "{" << std::endl
       << lunchbox::indent << "type          " << desc.type << std::endl
