...
public:
    Object(const size_t nChildren, size_t index)
    {
        ...
        size_t nGrandChildren =
            nChildren > treeWidth ? nChildren - treeWidth : 0;
...
{
public:
    explicit OperationIF(const std::string& name)
        : name_(name)
    {
    }
    virtual ~OperationIF() {}
...
public:
    ...
    void print(const float time) final
    {
        ...
        const float mbps =
            objectSize / 1024.0f / 1024.0f * nOps / time * 1000.f;
...
public:
    ...
    void print(const float time) final
    {
        ...
        const float mbps =
            objectSize / 1024.0f / 1024.0f * nObjects * nOps_ / time * 1000.f;
...
public:
        ...
        , canChange_(true)
    {
        ...
        registerCommand(CMD_NODE_PARAMS,
                        co::CommandFunc<LocalNode>(this,
                                                   &LocalNode::cmdParams_),
                        0);
        registerCommand(CMD_NODE_SETUP,
                        co::CommandFunc<LocalNode>(this, &LocalNode::cmdSetup_),
                        0);
        registerCommand(CMD_NODE_OPERATION,
                        co::CommandFunc<LocalNode>(this,
                                                   &LocalNode::cmdOperation_),
                        0);
    }
...
int main(int argc, char** argv)
{
    ...
    try // command line parsing
    {
        ...
        options.add_options()("help,h",
            ...
            "object size")("numObjects,n", po::value<uint64_t>(&nObjects),
                           "number of objects");
